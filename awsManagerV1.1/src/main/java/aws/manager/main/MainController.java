...
import aws.manager.util.login.LoginSessionUtil;
...
public class MainController {
    ...
    /**
     * 
     * javaFX 의 일반적인 initialize 을 작성하면된다.
     * 
     * ------------------------------------
    ...
    public void initialize() {
            ...
            s3Controll();
            //첫 로드 시 공개키 및 암호키 셋팅
            SCAccessKeyText.setText(LoginSessionUtil.getLoginSession().getScAccessBaseKeyText());
            SCSecurityKeyText.setText(LoginSessionUtil.getLoginSession().getScSecurityBaseKeyText());
                ...
                public void handle(ActionEvent event) {
                 ...
                 //업로드 된 후 파일명
                    ...
                    if(objectName.equals("")||bucketName.equals("")){
                        ...
                        try{
                            //파일 이름 중복되면 덮어쓰기
                            LoginSessionUtil.getLoginSession().getS3ClientSession().putObject(new PutObjectRequest(bucketName, objectName, createSampleFile(dir)));
                            bucketListTree();
         ...
         */
          private void startAction(String tabName) {
              ...
              if(!tabName.equals("Security Credential")){
                  if(LoginSessionUtil.getLoginSession().getEC2ClientSession() == null){
                      flag = false;
                  }
              }
              if(flag){
              ...
              switch (tabName) {
                case "Security Credential":
                    ...
                    LoginSessionUtil.getLoginSession().setEC2ClientSession(new AmazonEC2Client(credentials));
                    LoginSessionUtil.getLoginSession().setS3ClientSession(new AmazonS3Client(credentials));
                    try{
                        //엑세스키 검사
                        LoginSessionUtil.getLoginSession().getS3ClientSession().listBuckets();
                        ...
                        LoginSessionUtil.getLoginSession().getEC2ClientSession().setRegion(region);
//                      System.out.println();

                        createAlert("완료","증명되었습니다");
                        resetText("Security Credential");
                    }catch(AmazonS3Exception ase){
                        LoginSessionUtil.getLoginSession().setEC2ClientSession(null);
                        LoginSessionUtil.getLoginSession().setS3ClientSession(null);
                ...
                case "Security Group":
                    ...
                    try {
                        CreateSecurityGroupResult createSecurityGroupResult=LoginSessionUtil.getLoginSession().getEC2ClientSession().createSecurityGroup(csgr);
                ...
                case "Inbound Setting":
                    ...
                    try{
                        AuthorizeSecurityGroupIngressResult result=LoginSessionUtil.getLoginSession().getEC2ClientSession().authorizeSecurityGroupIngress(authorizeSecurityGroupIngressRequest);
                ...
                case "Launch EC2":
                    ...
                    runInstancesRequest.withImageId(LaunchAMIId)
                     ...
                     try{
                         LoginSessionUtil.getLoginSession().getEC2ClientSession().runInstances(runInstancesRequest);
                ...
                case "Key Pair생성":
                    ...
                    //서버에 생성 요청
                    try{    
                        LoginSessionUtil.getLoginSession().getEC2ClientSession().createKeyPair(createKeyPairRequest);
...
//              System.out.println("Listing buckets");
              for (Bucket bucket : LoginSessionUtil.getLoginSession().getS3ClientSession().listBuckets()) {
...
//                  System.out.println(" - " + bucketName);
        ...
        public List<String> listObject(String bucketName){
            List<String> result = new ArrayList<>();
          if((LoginSessionUtil.getLoginSession().getS3ClientSession().doesBucketExist(bucketName))){
              //System.out.println("Listing objects");
              ObjectListing objectListing = LoginSessionUtil.getLoginSession().getS3ClientSession().listObjects(new ListObjectsRequest().withBucketName(bucketName));
      ...
      private void s3Controll() {
          ...
          CreateBucketBtn.setOnAction(new EventHandler<ActionEvent>() {
            ...
            public void handle(ActionEvent event) {
                ...
                try{
                   if(!(LoginSessionUtil.getLoginSession().getS3ClientSession().doesBucketExist(bucketName))){
                       LoginSessionUtil.getLoginSession().getS3ClientSession().createBucket(bucketName);   
                        bucketListTree();
...
//                    System.out.println(" - removing objects from bucket");
                    ObjectListing object_listing = LoginSessionUtil.getLoginSession().getS3ClientSession().listObjects(bucketName);
                    while (true) {
                        for (Iterator<?> iterator =
                                object_listing.getObjectSummaries().iterator();
                                iterator.hasNext();) {
                            S3ObjectSummary summary = (S3ObjectSummary)iterator.next();
                            LoginSessionUtil.getLoginSession().getS3ClientSession().deleteObject(bucketName, summary.getKey());
                        }
                        // more object_listing to retrieve?
                        if (object_listing.isTruncated()) {
                            object_listing = LoginSessionUtil.getLoginSession().getS3ClientSession().listNextBatchOfObjects(object_listing);
                        } else {
                            break;
                        }
                    }
                    LoginSessionUtil.getLoginSession().getS3ClientSession().deleteBucket(bucketName);
                    bucketListTree();
         ...
         DeleteFileBtn.setOnAction(new EventHandler<ActionEvent>() {
            ...
            //DWDEBucketText
             ...
             objectName =DWDEFileText.getText();
              ...
              try{
                  if((LoginSessionUtil.getLoginSession().getS3ClientSession().doesBucketExist(bucketName))){
                      if(LoginSessionUtil.getLoginSession().getS3ClientSession().doesObjectExist(bucketName, objectName)){
                          LoginSessionUtil.getLoginSession().getS3ClientSession().deleteObject(bucketName, objectName);
                          bucketListTree();
                 ...
                 if(bucketName.equals("")||bucketName == null||objectName.equals("")||objectName == null){
                     ...
                     try{
                         if((LoginSessionUtil.getLoginSession().getS3ClientSession().doesObjectExist(bucketName, objectName))){
                             ...
                             int count = 0;
                             //클라이언트로 부터 파일 받아옴
                             S3Object object = LoginSessionUtil.getLoginSession().getS3ClientSession().getObject(new GetObjectRequest(bucketName, objectName));
